...
import type { Gestante } from '@/features/gestantes/gestantes.schema'

interface GestantesTableProps {
  search: {
    value: string
...
export function GestantesTable({
  search,
  pagination,
  data,
}: GestantesTableProps) {
  ...
  const columns: ColumnDef<Gestante>[] = useMemo(() => [
    ...
    {
      accessorKey: 'antecedentesObstetricos',
      header: 'Antecedentes',
      cell: ({ getValue }) => {
          ...
          <DropdownMenuContent align="end" className="w-40">
            {table
              ...
              .map((column) => (
                ...
                >
                  {column.id === 'antecedentesObstetricos' ? 'Antecedentes' : column.id}
                </DropdownMenuCheckboxItem>
              ))}
          </DropdownMenuContent>
        </DropdownMenu>
